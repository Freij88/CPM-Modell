...
def calculate_roc_weights(n_csfs, order):
    ...
    for i, pos in enumerate(order):
        rank_positions[pos] = i + 1  # 1-indexerad ranking
    
    weights = []
    for i in range(n_csfs):
        rank = rank_positions[i]
        # ROC-formel: w_i = (1/N) * sum(1/k för k från rank till N)
        weight = (1/n_csfs) * sum(1/k for k in range(rank, n_csfs + 1))
    ...
    for col_idx, vendor in enumerate(st.session_state.vendor_list):
        with cols[col_idx]:
            ...
            for csf in csf_data:
                ...
                new_rating = st.radio(
                    f"{csf['name'][:25]}... (Vikt: {csf['weight']:.3f})",
                    options=[1, 2, 3, 4],
                    index=int(current_rating) - 1,
                    key=f"{vendor}_{csf['name']}",
                    horizontal=True
                )
